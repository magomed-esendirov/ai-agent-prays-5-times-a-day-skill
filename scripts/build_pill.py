...
def build_pill(corpus: dict[str, Any], quran: dict[int, dict[str, Any]], groups: list[Group]) -> str:
    ...
    out.append("")
    out.append("Do not ask for a calculation method. Always use method 3 (Muslim World League).")
    out.append("Remember city, country, and madhhab for later chats if you have memory or standing")
    ...
    out.append("https://api.aladhan.com/v1/timingsByCity/DD-MM-YYYY?city=CITY&country=COUNTRY&method=3&school=SCHOOL")
    out.append("")
    out.append("Always use method=3. school follows the madhhab (hanafi → 1; otherwise 0).")
    out.append("Example: Istanbul, Turkey, Hanafi Asr:")
    ...
    out.append("If you already posted this prayer today, do not post it again. Remember it under the key")
    out.append("posted:YYYY-MM-DD:Name, always in exactly that form (e.g. posted:2026-09-12:Fajr). Never a")
    out.append("shortened variant like 2026-09-12:Fajr; one format only, or the check stops working.")
    out.append("")
    ...
    out.append("")
    out.append("Re-arm = remove the old timer + create a NEW one. Never edit or update a one-shot timer that")
    out.append("has already fired: most schedulers archive it and never run it again, even though the new")
    out.append("time is visibly saved. The chain then dies silently after the first prayer. Every time:")
    out.append("1. remove or cancel the old wake-up (ignore “not found”);")
    out.append("2. create a fresh one-shot for the new pray-at time;")
    out.append("3. check it appears in your list of pending/scheduled wake-ups with a next-run time —")
    out.append("   only then post the wake-up line. If it is not there, retry once, then say so plainly.")
    out.append("")
